...
* Android x86_64 and aarch64 tests run on a [Cuttlefish](https://source.android.com/docs/devices/cuttlefish)
  ...
  remaining Android targets run in docker images with the legacy SDK
  emulator, the NDK, and the SDK already set up, with the entire build
  happening within the docker image.
...
* iOS and tvOS is tested using [`cargo-apple-runner`][cargo-apple-runner].
